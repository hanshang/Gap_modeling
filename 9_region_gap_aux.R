# index: indicator of prefectures
# N_M_CDF: national male CDF
# N_F_CDF: national female CDF
# fh: forecast horizon
# fmethod: forecasting method

region_gap_fore_fun_interval <- function(index, N_M_CDF, N_F_CDF, fh, fmethod)
{
    N_nrow = nrow(N_M_CDF)
    N_ncol = ncol(N_M_CDF)
    ...
    if(any(N_M_CDF <= 0))
    {
        N_M_CDF = replace(N_M_CDF, which(N_M_CDF == 0), 10^-8)
    }
    ...
    Japan_male_pop_CDF_forecast   = c(invlogit(forecast(ftsm(fts(ages[1:(N_nrow - 1)], logit(N_M_CDF[1:(N_nrow - 1),tail(1:N_ncol, N_prefecture_ncol)]))), h = fh, method = fmethod)$mean$y[,fh]), 1)

    # region gap via Fisher Z transformation
    
    subnational_region_F_diff_CDF_forecast = FisherZInv(forecast(ftsm(fts(ages, FisherZ(subnational_region_F_diff_CDF[[index]][,1:N_prefecture_ncol]))), h = fh, method = fmethod)$mean$y[,fh])
    subnational_region_M_diff_CDF_forecast = FisherZInv(forecast(ftsm(fts(ages, FisherZ(subnational_region_M_diff_CDF[[index]][,1:N_prefecture_ncol]))), h = fh, method = fmethod)$mean$y[,fh])
    ...
    return(list(female_fore = subnational_female_pop_PDF_forecast, male_fore = subnational_male_pop_PDF_forecast))
}
...
region_gap_fore_fun_conformal <- function(prefecture_index, national_M_CDF, national_F_CDF, 
                                          PDF_M_holdout, PDF_F_holdout, horizon,
                                          fore_method, length_test_data, alpha)
{
    n_row = nrow(national_M_CDF)
    ...
    for(iwk in 1:(length_test_data + 1 - horizon))
    {
        ...
        {
            dum = region_gap_fore_fun_interval(index = prefecture_index, 
                                               N_M_CDF = national_M_CDF[,tail(1:77, n_col)[1:ij]], 
                                               N_F_CDF = national_F_CDF[,tail(1:77, n_col)[1:ij]], 
        ...
        dum = region_gap_fore_fun_interval(index = prefecture_index, 
                                           N_M_CDF = national_M_CDF[,tail(1:77, n_col)[1:(n_col - (length_test_data + 1) + iwk)]],
